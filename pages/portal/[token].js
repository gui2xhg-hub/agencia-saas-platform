...
export default function ClientPortal() {
  ...
  const fetchPortalData = async () => {
    ...
    try {
      ...
      const [tRes, contRes, invRes, pRes, mRes] = await Promise.all([
        ...
        supabase.from('agency_metrics').select('*').eq('client_id', cData.id).order('month_year', { ascending: false }).limit(1).maybeSingle()
      ]);
      ...
      if (mRes.data) setLatestMetric(mRes.data);
    } catch (e) {
  ...
  if (!client) return <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center font-sans"><h1 className="text-xl font-bold text-red-500">Portal Inválido</h1></div>;
    ...
    <div className="min-h-screen font-sans p-4 sm:p-8 max-w-6xl mx-auto space-y-6 pb-20" style={{ backgroundColor: secondaryColor, color: textColor }}>
      ...
      <header className="flex items-center space-x-4 border border-white/10 p-6 rounded-3xl shadow-xl" style={{ backgroundColor: cardBgColor }}>
        ...
        <div>
          ...
          <p className="text-xs font-bold mt-0.5" style={{ color: priceColor }}>Portal do Cliente • Cronograma & Métricas</p>
        </div>
      </header>
      ...
      {latestMetric ? (
        <section className="space-y-3">
          <div className="flex justify-between items-center">
            <h2 className="text-sm font-extrabold" style={{ color: priceColor }}>📊 Métricas Reais ({latestMetric.month_year})</h2>
            <span className="text-xs text-green-400 font-bold bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20">{latestMetric.growth_percentage} este mês</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="border border-white/10 p-4 rounded-2xl space-y-1 shadow-lg" style={{ backgroundColor: cardBgColor }}>
              ...
              <span className="text-xl font-black text-blue-400">{latestMetric.reach}</span>
            </div>
            <div className="border border-white/10 p-4 rounded-2xl space-y-1 shadow-lg" style={{ backgroundColor: cardBgColor }}>
              <span className="text-[10px] font-bold opacity-60 block">💬 Engajamento</span>
              <span className="text-xl font-black text-purple-400">{latestMetric.engagement}</span>
            </div>
            <div className="border border-white/10 p-4 rounded-2xl space-y-1 shadow-lg" style={{ backgroundColor: cardBgColor }}>
              <span className="text-[10px] font-bold opacity-60 block">👥 Novos Seguidores</span>
              <span className="text-xl font-black text-green-400">{latestMetric.followers_gained}</span>
            </div>
            <div className="border border-white/10 p-4 rounded-2xl space-y-1 shadow-lg" style={{ backgroundColor: cardBgColor }}>
              <span className="text-[10px] font-bold opacity-60 block">🚀 Impressões Anúncios</span>
              <span className="text-xl font-black text-pink-400">{latestMetric.ad_impressions}</span>
            </div>
          </div>
        </section>
      ) : (
        <div className="bg-black/20 border border-white/5 p-4 rounded-2xl text-xs text-center opacity-60">
          Nenhum relatório de métricas cadastrado pela agência até o momento.
        </div>
      )}
      ...
      {contract && (
        <section className="border border-white/10 p-6 rounded-3xl space-y-3 shadow-xl" style={{ backgroundColor: cardBgColor }}>
          <h2 className="text-sm font-extrabold" style={{ color: priceColor }}>🎁 Seu Plano & Entregáveis ({contract.contract_name})</h2>
      ...
      <section className="space-y-4">
        <div className="border border-white/10 p-4 rounded-3xl flex justify-between items-center shadow-xl" style={{ backgroundColor: cardBgColor }}>
          ...
          <div className="text-center">
            <span className="text-[10px] uppercase font-bold opacity-60 block">Cronograma de Conteúdos</span>
      ...
      {/* MODAL PARA DETALHES E APROVAÇÃO */}
      {selectedPost && (
